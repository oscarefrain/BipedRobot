...
*******************************************************************************
        - robSimpleInitialStep:
          This program computes and displays the very initial step of the
          robot using the right leg as the moving leg (the fixed lef is the
          left one)
*******************************************************************************/
...
static double PreComputedAngleR[NINIT_T*TLENG][6], PreComputedAngleL[NINIT_T*TLENG][6];  // Variables for the joint angles, TLENG: # of different values in time
...
void start()
{
    ...
    Traj::rComputeInitialStep(PreComputedAngleL, PreComputedAngleR, target_angleL, target_angleR,
                              InitStepLength);
}
...
static void simLoop (int pause)
{
    ...
    if (!pause)					                // If “pause” key is not pressed
    {
        ...
        if(numSimSteps%10 == 0 && jointAngleStep < NINIT_T*TLENG)
        {
            for(int i=0;i<MAX_JOINTS;i++)
